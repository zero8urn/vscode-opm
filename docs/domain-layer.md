...
This document extracts the minimal, reusable architecture used by GitLens for the `src/domain` layer and explains how you can use it as the canonical domain abstraction when building a smaller VS Code extension.
...
Created from the GitLens architecture for a compact, reusable `src/git` layer to bootstrap new VS Code extensions.
